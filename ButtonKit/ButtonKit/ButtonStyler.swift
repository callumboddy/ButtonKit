...
extension UIButton {
    public func style(color: ColorSchemeColor = .Default, style: ButtonStyle = .Flat, radius: ButtonRadius = .Rounded) {
        ButtonStyler.style(self, color: color, style: style, radius: radius)
    }
    
    public func effect(style: ButtonEffectStyle) {
        ButtonEffect.applyEffectForEffectStyle(self, effect: style)
    }
}
